...
def fc2csv(in_file, osm_download_link, output_file):
    ...
    if osm_download_link:
        osm_download_link_data = gdf.apply(lambda y: osm_download_link_(y.geometry.bounds), axis=1)
        gdf.insert(gdf.columns.__len__() - 1, "osm_download_link", osm_download_link_data)
