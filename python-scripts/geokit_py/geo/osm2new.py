...
def osm2new(input_osm, output_osm):
    ...
    for relation in relations:
        ...
        if "user" in relation.attrib:
            del relation.attrib["user"]
        if "version" in relation.attrib:
            del relation.attrib["version"]
        if "timestamp" in relation.attrib:
            del relation.attrib["timestamp"]
        if "changeset" in relation.attrib:
            del relation.attrib["changeset"]
        if "uid" in relation.attrib:
