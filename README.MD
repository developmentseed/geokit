...
alias geokit='docker run --rm -v ${PWD}:/app developmentseed/geokit:latest geokit'
```
...
```
cd geokit/
docker run --rm -v ${PWD}:/app developmentseed/geokit:latest bash
```
...
```
geokit point2tile <File.geojson> --zoom=17 --buffer=0.2 > tiles.geojson
```
